...
def mini_batches_train(X_msg, X_code, Y, mini_batch_size=64, seed=1):
    m = X_msg.shape[0]  # number of training examples
    mini_batches = list()
    
    # Step 1: No shuffle (X, Y) Normal
    shuffled_X_msg, shuffled_X_code, shuffled_Y = X_msg, X_code, Y
    ...
    for k in range(0, num_complete_minibatches):
        ...
        #print(mini_batches)
        #random.seed(133)
        #mylist = ["apple", "banana", "cherry","test","pfirsich","apfelsine","drachenfrucht"]  
        #print(random.sample(mylist, k=4))
        #secondlist= mylist.sort()  
    return mini_batches